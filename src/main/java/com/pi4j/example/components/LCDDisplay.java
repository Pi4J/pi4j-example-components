...
import com.pi4j.example.components.I2CBus;
...
public class LCDDisplay extends Component {
    ...
    public void setDisplayBacklight(boolean state) {
        i2CBus.setAndWritePin(LCD_LIGHT, state);
    }
    ...
    protected void write(int b, boolean charMode) {
        b &= 0xFF;
        i2CBus.setAndWritePin(LCD_RS, charMode);

        // high nibble
        i2CBus.setPin(LCD_D4, (b & 0b0001_0000) > 0);
        i2CBus.setPin(LCD_D5, (b & 0b0010_0000) > 0);
        i2CBus.setPin(LCD_D6, (b & 0b0100_0000) > 0);
        i2CBus.setPin(LCD_D7, (b & 0b1000_0000) > 0);
        i2CBus.writePins();
        i2CBus.pulsePin(LCD_EN, 1);

        // low nibble
        i2CBus.setPin(LCD_D4, (b & 0b0000_0001) > 0);
        i2CBus.setPin(LCD_D5, (b & 0b0000_0010) > 0);
        i2CBus.setPin(LCD_D6, (b & 0b0000_0100) > 0);
        i2CBus.setPin(LCD_D7, (b & 0b0000_1000) > 0);
        i2CBus.writePins();
        i2CBus.pulsePin(LCD_EN, 1);
    }

    /**
     * Get the current i2CBus instance
     *
     * @return i2CBus Instance of this LCD Display
     */
    protected I2CBus geti2CBus() {
        return this.i2CBus;
    }

    /**
     * i2CBus IO Configuration makes pins to inputs or outputs
     */
    private static final byte i2CBus_IO_CONFIG = 0x00;
